    ...
    categories: [
      ...
      },
      {
        ...
        items: [
          ...
          { name: "Cloudflare", tag: "Stretch", url: "https://cloudflare.com/careers", note: "Heavy infrastructure overlap with your SQS/queue work" },
...
export default function Roadmap() {
  ...
  // completedItems stores which cards you've marked done
  // It reads from localStorage on first load so progress is saved
  const [completedItems, setCompletedItems] = useState(() => {
    const saved = localStorage.getItem("ts-completed");
    return saved ? JSON.parse(saved) : {};
  });

  // toggleItem runs when you click MARK DONE or UNDO on a card
  // It flips the item between complete and incomplete, then saves to localStorage
  const toggleItem = (itemName) => {
    setCompletedItems(prev => {
      const updated = { ...prev };
      if (updated[itemName]) {
        delete updated[itemName]; // undo if already done
      } else {
        updated[itemName] = true; // mark as done
      }
      localStorage.setItem("ts-completed", JSON.stringify(updated));
      return updated;
    });
  };
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .mark-btn { transition: all 0.15s ease; }
      ...
      <div style={{ display: "flex", height: "calc(100vh - 73px)", overflow: "hidden" }}>
              ...
              {phase.categories.map(cat => (
                <div key={cat.name} style={{ marginBottom: 28 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    {cat.items.map(item => {
                      const isDone = !!completedItems[item.name];
                      return (
                        <div
                          key={item.name}
                          className="item-card"
                          ...
                          style={{
                            ...
                            border: `1px solid ${isDone ? "#22c55e" : hoveredItem === item.name ? phase.border : "#1E2A3A"}`,
                            background: isDone ? "rgba(34,197,94,0.08)" : hoveredItem === item.name ? phase.bg : "rgba(255,255,255,0.02)",
                            transition: "all 0.15s ease"
                          }}>

                          {/* Item name + tag */}
                          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 6 }}>
                            <span style={{ fontSize: 13, fontWeight: 600, color: isDone ? "#22c55e" : "#D0D8E8", letterSpacing: 0.3 }}>
                              {isDone ? "✓ " : ""}{item.name}
                            </span>
                          ...
                          {/* Note */}
                          <div style={{ fontSize: 10, color: "#4A5A6A", lineHeight: 1.6, marginBottom: 10 }}>
                            {item.note}
                          </div>

                          {/* Bottom row: MARK DONE button + link */}
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            ...
                            <button
                              ...
                              style={{
                                ...
                                fontWeight: 600,
                                letterSpacing: 1,
                                border: `1px solid ${isDone ? "#22c55e" : "#2A3A4A"}`,
                                borderRadius: 3,
                                cursor: "pointer",
                                background: isDone ? "rgba(34,197,94,0.15)" : "transparent",
                                color: isDone ? "#22c55e" : "#4A5A6A",
                              }}
                            >
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              style={{ fontSize: 9, color: phase.color, opacity: hoveredItem === item.name ? 0.8 : 0.3, textDecoration: "none" }}
                            >
        ...
        <div style={{
            ...
            <div style={{ marginTop: 10 }}>
              {["VS Code", "GitHub", "SQS Queue", "ML Classifier", "API Layer"].map((step, i) => (
                <div key={step} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0" }}>
                  ...
                  <span style={{ fontSize: 10, color: i < 2 ? "#10B981" : "#4A5A6A" }}>{step}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginTop: 20, padding: 14, borderRadius: 4, border: "1px solid rgba(168,85,247,0.3)", background: "rgba(168,85,247,0.06)" }}>
            <div style={{ fontSize: 10, color: "#4A5A6A", lineHeight: 1.6 }}>
              Ask for a 30-min monthly mentorship call. Show this project in 3 months. Ask for referral at month 9.
            </div>
          </div>
        </div>
      </div>
